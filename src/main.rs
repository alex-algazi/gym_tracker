...
// - (subscribe Member Subscription) add subscription to existing member
// - (purchase Member Purchase) add purchase to existing member
// - (members) check member list
// - (subscriptions Member) check subscription list of member
// - (purchases Member) ckeck purchase list of member
...
fn main() {
    ...
    loop {
        ...
        if first_word == "new" {
            let (mut first, mut last, mut mail, mut gender_raw) = ( String::new(), String::new(), String::new(), String::new() );
            ...
            println!("User's gender? Enter 1 for \"Male\", 2 for \"Female\", 3 for \"Non-Binary\", or you can type something else that describes you best! (Also feel free to leave blank)");
            io::stdin().read_line(&mut gender_raw).expect("Invalid command");
            ...
            members.push(Member {
                ...
                gender:
                    if gender_raw.trim() == "1" { Gender::Male }
                    else if gender_raw.trim()  == "2" { Gender::Female }
                    else if gender_raw.trim()  == "3" { Gender::NonBinary }
                    else if gender_raw.trim()  == "" { Gender::PreferNotToSay }
                    else { Gender::Other(gender_raw.trim().into()) },
            ...
            if let Ok(int_id) = uid.trim().parse::<u32>() {
                ...
                if let Some(user_index) = member_iter.position(|usr| usr.member_id == int_id) {
                    ...
                    println!("Hello, {}. What service would you like to subscribe to? Enter 1 for Mandy's Yoga Mondays, 2 for Monthly Scuba Lessons, 3 for Willy's Weight Training", members[user_index].first_name);
                    io::stdin().read_line(&mut serv).expect("Invalid command");
                    if serv.trim()  == "1" {
                        if !members[user_index].subscriptions.contains(&Service::YogaMondays) {
                        ...
                        if !members[user_index].subscriptions.contains(&Service::ScubaLessons) {
                            members[user_index].subscriptions.push(Service::ScubaLessons);
                        } else {
                            println!("You are already subscribed to that service!");
                        }
                    }
                    else if serv.trim()  == "3" {
                        if !members[user_index].subscriptions.contains(&Service::WeightTraining) {
                            members[user_index].subscriptions.push(Service::WeightTraining);
                        } else {
                            println!("You are already subscribed to that service!");
                        }
                    }
            ...
            if let Ok(int_id) = uid.trim().parse::<u32>() {
                ...
                if let Some(user_index) = member_iter.position(|usr| usr.member_id == int_id) {
                    ...
                    println!("Hello, {}. What item would you like to purchase? Enter 1 for Pepsi, 2 for Diet Pepsi, 3 for Sierra Mist, 4 for Mountain Dew, 5 for Powerade, 6 for Coffee, 7 for Latte", members[user_index].first_name);
            ...
            if let Ok(int_id) = uid.trim().parse::<u32>() {
                if let Some(user) = members.clone().into_iter().find(|member| member.member_id == int_id) {
