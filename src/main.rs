...
fn new_member(member_list: &mut Vec<Member>) {
...
fn user_action(member_list: &mut Vec<Member>, is_subscription: bool) {
    ...
    if let Ok(int_id) = uid.trim().parse::<u32>() {
        let mut member_iter = member_list.clone().into_iter();
        if let Some(user_index) = member_iter.position(|usr| usr.member_id == int_id) {
            if is_subscription {
                ...
                println!(
                    ...
                    member_list[user_index].first_name
                );
                io::stdin().read_line(&mut serv).expect("Invalid command");
                if serv.trim() == "1" {
                    if !member_list[user_index]
                        .subscriptions
                        .contains(&Service::YogaMondays)
                    {
                        member_list[user_index]
                            .subscriptions
                            .push(Service::YogaMondays);
                    } else {
                ...
                if itm.trim() == "1" {
                    member_list[user_index].purchases.push(Item::Pepsi);
                } else if itm.trim() == "2" {
                    member_list[user_index].purchases.push(Item::DietPepsi);
                } else if itm.trim() == "3" {
                    member_list[user_index].purchases.push(Item::SierraMist);
                } else if itm.trim() == "4" {
                    member_list[user_index].purchases.push(Item::MountainDew);
                } else if itm.trim() == "5" {
                    member_list[user_index].purchases.push(Item::Powerade);
                } else if itm.trim() == "6" {
                    member_list[user_index].purchases.push(Item::Coffee);
                } else if itm.trim() == "7" {
                    member_list[user_index].purchases.push(Item::Latte);
                } else {
...
fn get_member_list(member_list: &Vec<Member>) {
    let user_iter = member_list.clone().into_iter();
    for member in user_iter {
        println! {"{} - {} {} (Gender: {}), {}", member.member_id, member.first_name, member.last_name, member.gender, member.email};
    }
}

fn get_user_info(member_list: &Vec<Member>, get_subs: bool) {
    let mut uid = String::new();
